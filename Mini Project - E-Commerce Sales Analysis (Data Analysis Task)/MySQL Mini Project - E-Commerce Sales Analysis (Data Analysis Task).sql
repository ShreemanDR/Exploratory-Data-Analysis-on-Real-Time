-- Creating a Database: E-Commerce_Sales :
Create Database ECommerce_Sales;
Use ECommerce_Sales;

-- TASK1 - Creating Tables: Customers, Products, Orders & OrderDetails:
...
show tables; -- checking whether the Tables are created
             
-- TASK2 - Altering the Products table to add a new column = stock_quantity:
ALTER TABLE Products
ADD COLUMN stock_quantity int;
select * from Products; -- checking

-- TASK3 - Inserting data into the Customers, Products, Orders, and OrderDetails Tables:
...
Select * from OrderDetails; -- checking

-- Task4 - Aggregation Operators: Calculate the total sales for each product category :
select category, SUM(total_price) as Total_Sales_for_each_Product_Category FROM Customers
INNER JOIN Orders ON Customers.customer_id = Orders.customer_id
INNER JOIN OrderDetails ON Orders.order_id = OrderDetails.order_id
INNER JOIN Products ON OrderDetails.product_id = Products.product_id
GROUP BY category;

-- Task5 - Perform Joins: Detailed list of orders (customer & product names,and the total price for each order)
   -- a) JOIN To join the tables together as a new table :
CREATE TABLE Customer_Order_Product_Details AS
SELECT
    Customers.customer_id, Customers.name AS customer_name, Customers.email, Customers.join_date,
    Orders.order_id,Orders.order_date,
    OrderDetails.order_detail_id, OrderDetails.product_id,
    Products.name AS product_name, Products.category, Products.price, Products.stock_quantity,
    OrderDetails.quantity, OrderDetails.total_price
FROM Customers
INNER JOIN Orders ON Customers.customer_id = Orders.customer_id
INNER JOIN OrderDetails ON Orders.order_id = OrderDetails.order_id
INNER JOIN Products ON OrderDetails.product_id = Products.product_id;
select * from Customer_Order_Product_Details; -- checking
	-- b) Retrieve a detailed list of orders including customer & product names and the total price for each order :
select order_id, order_date, customer_id, customer_name, product_name, quantity as order_quantity, total_price
from Customer_Order_Product_Details;

-- Task6 - SQL Queries:
-- 1) Write a query to find the total number of orders placed by each customer?
select count(distinct customer_name) from Customer_Order_Product_Details; -- checking
SELECT customer_id, customer_name, COUNT(order_id) AS total_orders FROM Customer_Order_Product_Details
GROUP BY customer_id, customer_name;

-- 2) Write a query to find the total revenue generated from each product category?
Select Distinct Category from Customer_Order_Product_Details; -- checking
Select Category, sum(total_price) as Total_Revenue from Customer_Order_Product_Details
group by Category;

-- 3) Write a query to find the top 3 customers by total spending?
Select Customer_name as Top_3_customers, total_price as Total_Spending from Customer_Order_Product_Details
order by total_price desc
limit 3;

-- 4) Write a query to get the details of all orders placed in the last month?
select count(Distinct order_date) from Customer_Order_Product_Details; -- checking
Select order_id, customer_name, order_date, product_name, total_price from Customer_Order_Product_Details
WHERE order_date>='2023-02-01' and order_date<='2023-02-28'; -- Last month as February (Current March)
--                                             (or)
Select order_id, customer_name, order_date, product_name, total_price from Customer_Order_Product_Details
order by order_id desc
limit 30; -- last month in days (30 days)
